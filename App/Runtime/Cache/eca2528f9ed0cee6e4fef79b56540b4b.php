...
<body>
	...
	<div class="header">
		<h2>运动会管理系统-比赛成绩</h2>
<select data-am-selected onchange="self.location.href=options[selectedIndex].value">
  <option value="/index.php/Index/result/eid/0">请选择</option>
    <option value="/index.php/Index/result/">全部</option>
<?php if(is_array($eventList)): $i = 0; $__LIST__ = $eventList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="/index.php/Index/result/eid/<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
	</div>
...
<table class="am-table">
    ...
    <?php $i=1 ?>
    	<?php if(is_array($scoreList)): $i = 0; $__LIST__ = $scoreList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td><a class="am-btn am-btn-primary am-btn-xs"  onclick="get(<?php echo ($vo["id"]); ?>)">领取奖品</a></td>
           	<td>			

           	<?php if(($vo['isGet'] == 1)): ?><a class="am-btn am-btn-warning am-btn-xs" >已领取</a>
<?php else: ?>未领取<?php endif; ?>
